...
(function () {
  ...
  window.SMG.formatDate = function (isoDate) {
    if (!isoDate) return "";
  ...
  window.SMG.categoryName = function (slug) {
  ...
  window.SMG.renderPostCards = function (containerId, posts) {
    ...
    el.innerHTML = posts
      .map(function (p) {
        return (
          ...
          window.SMG.categoryName(p.category) +
          "</span>" +
          "<h3><a href=\"/posts/" +
          p.slug +
          '/">' +
          ...
          "<span>발행 " +
          window.SMG.formatDate(p.publishedAt) +
          "</span>" +
          "<span>수정 " +
          window.SMG.formatDate(p.updatedAt) +
          "</span>" +
          "</div>" +
          "</div>" +
          "</article>"
  ...
  window.SMG.renderColumnCards = function (containerId, columns) {
    var el = document.getElementById(containerId);
    if (!el) return;
    if (!columns.length) {
    ...
    el.innerHTML = columns
      .map(function (c) {
        return (
          ...
          '<div class="post-card-meta">' +
          "<span>" +
          window.SMG.formatDate(c.publishedAt) +
          "</span>" +
          "</div>" +
          "</div>" +
          "</article>"
