/**
 * 스마트머니 가이드 관리자 모드 (CMS-lite 데모)
 * 실제 데이터베이스/인증 시스템이 아니라, localStorage 기반의 편집 데모입니다.
 * 여기서 저장한 내용은 이 브라우저에만 남고, 실제 배포된 정적 페이지에는 반영되지 않습니다.
 */
(function () {
  var KEYS = {
    posts: "smg_admin_posts",
    columns: "smg_admin_columns",
    categories: "smg_admin_categories",
    settings: "smg_admin_settings"
  };
  ...
  function initAuthUI() {
    ...
    logoutBtn.addEventListener("click", function () {
      window.SMG_AUTH.logout();
      showLogin();
    });

    if (window.SMG_AUTH.isAdminLoggedIn()) {
      showShell();
    } else {
      showLogin();
    }
  ...
  function loadSettingsForm() {
    ...
    document.getElementById("setMainColor").value = s.mainColor || "#1E3A8A";
    document.getElementById("setSubColor").value = s.subColor || "#10B981";
  ...
  function initDataTools() {
    document.getElementById("exportBtn").addEventListener("click", function () {
      ...
      a.href = url;
      a.download = "smartmoney-guide-cms-data.json";
